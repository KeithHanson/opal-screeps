...
Opal.modules["war_supervisor"] = function(Opal) {
  ...
  return (function($base, $super, $parent_nesting) {
    ...
    var def = self.$$proto, $nesting = [self].concat($parent_nesting), TMP_WarSupervisor_tick$B_1;

    return Opal.defs(self, '$tick!', TMP_WarSupervisor_tick$B_1 = function() {
      var self = this;

      if ($truthy(Opal.const_get_relative($nesting, 'DEBUG'))) {
        return self.$puts("WarSupervisor Tick!")
        } else {
        return nil
      }
    }, TMP_WarSupervisor_tick$B_1.$$arity = 0)
  })($nesting[0], null, $nesting)
};
...
(function(Opal) {
  ...
  Opal.add_stubs(['$require', '$Native', '$tick!', '$each', '$[]', '$<<', '$nil?', '$length', '$==', '$to_i', '$puts', '$spawnCreep', '$ticksToLive']);
  
  self.$require("native");
  self.$require("war_supervisor");
  ...
  return (function($base, $super, $parent_nesting) {
    ...
    var def = self.$$proto, $nesting = [self].concat($parent_nesting), TMP_ScreepsEntry_loop_3;

    return Opal.defs(self, '$loop', TMP_ScreepsEntry_loop_3 = function $$loop() {
      var TMP_1, $a, $b, TMP_2, self = this, game = nil, creeps = nil;

      
      Opal.const_get_relative($nesting, 'WarSupervisor')['$tick!']();
      game = self.$Native(Game);
      creeps = [];
      $send(game['$[]']("creeps"), 'each', [], (TMP_1 = function(creep){var self = TMP_1.$$s || this;
if (creep == null) creep = nil;
      return creeps['$<<'](creep)}, TMP_1.$$s = self, TMP_1.$$arity = 1, TMP_1));
      if ($truthy(($truthy($a = ($truthy($b = creeps['$nil?']()) ? $b : creeps.$length()['$nil?']())) ? $a : creeps.$length().$to_i()['$=='](0)))) {
        
        if ($truthy(Opal.const_get_relative($nesting, 'DEBUG'))) {
          self.$puts("No creeps.")};
        game['$[]']("spawns")['$[]']("Spawn1").$spawnCreep([Opal.const_get_relative($nesting, 'WORK'), Opal.const_get_relative($nesting, 'CARRY'), Opal.const_get_relative($nesting, 'MOVE')], "Creep");
        } else {
        
        if ($truthy(Opal.const_get_relative($nesting, 'DEBUG'))) {
          self.$puts("" + (creeps.$length().$to_i()) + " creeps.")};
        $send(creeps, 'each', [], (TMP_2 = function(creep_name){var self = TMP_2.$$s || this;
if (creep_name == null) creep_name = nil;
        return self.$puts("" + "\t" + (creep_name) + ": " + (game['$[]']("creeps")['$[]'](creep_name).$ticksToLive()))}, TMP_2.$$s = self, TMP_2.$$arity = 1, TMP_2));
      };
      if ($truthy(Opal.const_get_relative($nesting, 'DEBUG'))) {
        return self.$puts("ScreepsEntry.loop() complete.")
        } else {
        return nil
      };
    }, TMP_ScreepsEntry_loop_3.$$arity = 0)
  })($nesting[0], null, $nesting);
})(Opal);
